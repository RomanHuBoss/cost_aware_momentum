...
def test_training_labels_use_direction_specific_executable_entry_stress() -> None:
    ...
    assert pair.loc["LONG", "entry_mid_proxy"] == pytest.approx(110.0)
    assert pair.loc["SHORT", "entry_mid_proxy"] == pytest.approx(110.0)
    assert pair.loc["LONG", "entry_price"] == pytest.approx(110.0 * 1.001)
    assert pair.loc["SHORT", "entry_price"] == pytest.approx(110.0 * 0.999)
