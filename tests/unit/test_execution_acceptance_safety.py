...
def test_acceptance_validator_rejects_current_entry_outside_signal_zone() -> None:
    """Acceptance validation must enforce the immutable decision-time entry band.

    The API layer already checks this before calling the validator. The validator
    itself is the safety boundary for fresh plan acceptance, so it must not accept
    a stale ACTIONABLE plan at a fresh executable price outside the model's
    decision-time support, even when the drift is price-favorable and all risk/RR
    checks still pass.
    """
    ...
    with pytest.raises(ValueError, match="outside entry zone"):
