...
@pytest.mark.asyncio
async def test_previous_hour_candle_cannot_publish_current_hour_signal(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    now = datetime.now(UTC)
    event_time = now.replace(minute=0, second=0, microsecond=0)
    prior_close = event_time - timedelta(hours=1)
    frame = pd.DataFrame([{"close_time": prior_close}])
    ...
    def must_not_evaluate_scenario(*_args, **_kwargs):
        raise AssertionError("a prior-hour feature window reached current-hour signal economics")
    ...
    monkeypatch.setattr(signals, "select_cost_aware_scenario", must_not_evaluate_scenario)
    ...
    assert published == []
    assert diagnostics["skip_counts"] == {"missing_decision_candle": 1}
