...
> Версия 1.52.25: часовой inference теперь ограниченно повторяется внутри неизменяемого 10-минутного окна, если первый проход завершился только из-за временно недоступной свечи, market context, ticker или instrument evidence. Policy/market/model/drift/economics rejects не повторяются; risk/quality gates не ослаблены. Alembic schema, `.env`, model artifacts, thresholds и advisory-only Bybit endpoint set не изменены.
...
- До запуска bootstrap trainer вычисляет необходимую часовую историю из feature warm-up, horizon, temporal split и holdout gates. При defaults требуется не менее 1206 уникальных часовых timestamps; это необходимое, но не достаточное условие при гэпах/невалидных свечах. Startup backfill по умолчанию запрашивает 1500 баров и paginates exchange kline pages больше 1000, чтобы clean install не застревал на первом 1000-bar page при уже доступной истории. Progressive history backfill отдельно запрашивает 7 open-interest pages × 200 hourly rows, потому что общий 2-page default, достаточный для kline/funding cycles, давал только около 400 OI hours и после point-in-time filtering оставлял около 326 development timestamps при required 366. На чистой dynamic-установке trainer фиксирует последний hash-validated execution-eligible universe snapshot, использует его как неизменяемый cohort для historical candle bootstrap и явно маркирует artifact `historical_frozen_dynamic_bootstrap`. Историческая membership не выдумывается: exact prospective snapshots продолжают копиться отдельно, а после достаточного span trainer автоматически переходит на `prospective_dynamic_replay`.
...
- Начиная с 1.52.25 ограниченное исключение из terminal-outcome правила применяется только к явно временным data-availability причинам (`missing_decision_candle`, incomplete context/ticker/spec evidence и родственные коды): до пяти повторов с существующим cooldown допускаются лишь внутри immutable publication window. Spread, entry-zone, model, drift и economics rejects остаются окончательными для данного часа и не пересматриваются в ожидании более выгодного рынка.
...
Начиная с 1.52.24 все финансовые/advisory чтения, detailed readiness/status и `/api/v1/events` требуют signed operator session либо заголовок `X-Operator-Token`. Для machine readiness probe задайте непустой `OPERATOR_API_TOKEN` и отправляйте его в этом заголовке. В `APP_MODE=production` обязательно `COOKIE_SECURE=true`; для локального HTTP paper/development допускается `false`. Logout через browser session требует совпадающий `X-CSRF-Token`; API-token authentication уже является explicit non-cookie authentication и не требует cookie CSRF.
...
Release 1.52.8 не добавляет migration, `.env` variables или API-breaking changes. После обновления перезапустите API/UI, worker и trainer. При ошибке `No direction-specific barrier labels could be built from PostgreSQL candles` операторский диалог trainer теперь показывает, что последняя попытка не смогла построить direction-specific TP/SL/TIMEOUT labels из PostgreSQL candles и ждёт retry/backfill, вместо общего сообщения об отсутствии причины ожидания.
