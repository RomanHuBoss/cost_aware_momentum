...
> Версия 1.48.0: pooled sparse interaction evidence дополнительно проходит leave-one-cell-out jackknife. После удаления каждой малой `symbol × direction × regime` ячейки остаток должен сохранять минимум пять сделок, положительную экономику и допустимую калибровку; одна удачная tiny-cell больше не может поддерживать весь sparse-pool.
