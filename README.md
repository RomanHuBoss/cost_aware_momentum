...
> Версия 1.38.0: background trainer теперь обязан обучаться на точном symbol cohort и временном cutoff, которые прошли preflight. Dynamic mode больше не заменяет ограниченный `AUTO_TRAIN_MAX_SYMBOLS` profile неограниченной загрузкой всех символов; фактическое post-feature покрытие повторно проверяется fail-closed перед promotion.
