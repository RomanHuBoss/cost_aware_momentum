...
> Версия 1.12.0: research pipeline прогрессивно загружает фактические Bybit funding settlements, привязывает денежные потоки к окну `(entry_time, exit_time]` и блокирует обучение при неполной settlement timeline. Будущие фактические ставки используются только в realized OOS PnL и не влияют на ex-ante выбор LONG/SHORT или actionability.
...
`MODEL_ENTRY_SPREAD_BPS` задаёт полный bid/ask spread stress для historical labels. Значение делится пополам вокруг следующего hourly open: LONG моделируется по ask-side proxy, SHORT — по bid-side proxy. Изменение этой переменной меняет label geometry; после обновления требуется обучить новый artifact. Артефакты со старой frictionless-open схемой отклоняются fail-closed. Версия 1.12.0 дополнительно требует `historical_funding_schema=bybit-settlement-timestamp-replay-v1`; artifacts 1.11.0 и старше необходимо переобучить после завершения funding-history backfill.
...
Worker синхронизирует read-only market/account data, instrument specifications, confirmed candles, ticker/funding snapshots и прогрессивно backfill-ит фактические funding settlement events вместе со свечной историей, после чего строит рекомендации. Неполные или устаревшие данные блокируют публикацию. Для hourly decision последняя confirmed свеча должна закрываться точно в `event_time`; окно с предыдущим часовым close не считается допустимо свежим даже при небольшом возрасте данных.
...
Research backtest не моделирует intrahorizon mark-to-market, полный historical order book, entry-zone/no-fill, partial fills, point-in-time funding forecast и задержку оператора и не является доказательством прибыльности. Фактическая settlement timeline реализована для realized costs; completeness пока проверяется по последнему известному funding interval инструмента.
Эти ограничения также означают, что прохождение unit tests или promotion gate не подтверждает live-edge: исторический research entry использует open первой часовой свечи после decision time, тогда как live plan использует более поздний executable bid/ask и ручное решение оператора. До накопления point-in-time liquidity snapshots, historical funding-forecast snapshots и operator-latency observations этот разрыв должен считаться существенным модельным риском.
