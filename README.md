...
> Версия 1.21.0: operator-selection denominator теперь строится только по рекомендациям, реально показанным в локальном UI. Карточка фиксируется после ≥50% видимости в активной вкладке в течение ≥1 секунды; first exposure сохраняется неизменяемо, проверяется SHA-256 и используется как chronological observation time. Низкое покрытие exposure блокирует IPSW-оценку.
