...
> Версия 1.35.5: hourly и universe-catchup inference теперь получают и сохраняют свежий Bybit ticker batch непосредственно перед публикацией сигналов. Общий market sync записывает новый ticker payload только после потенциально долгих orderbook/backfill операций; нулевой refresh блокирует inference, а stale-журнал содержит точный возраст и timestamps снимка.
