...
> Версия 1.8.24: пагинация справочника инструментов Bybit теперь fail-closed отклоняет повторяющийся cursor и невалидный тип страницы, не позволяя instrument sync бесконечно блокировать worker. Проверка внешних рекомендаций также подтвердила корректную point-in-time семантику признаков: решение использует уже закрытую часовую свечу, а label начинается со следующей.
