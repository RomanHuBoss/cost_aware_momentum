...
> Версия 1.51.1: release attestation теперь fail-closed проверяет обязательные governance/operations документы, наличие version-specific `PATCH_1.51.1.md` и iteration report, а также точное совпадение версии в `pyproject.toml`, `app/__init__.py` и README. Самосогласованный, но неполный `SHA256SUMS` больше не считается достаточным доказательством корректного релиза.
