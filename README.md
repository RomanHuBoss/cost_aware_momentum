...
> Версия 1.35.4: устранён batch-wide `409 Conflict` при регистрации UI-exposure, восстановлена настоящая идемпотентность browser retry и добавлена cross-ledger проверка экспериментальных свидетельств. Orderbook и read-only account snapshots теперь выбираются строго point-in-time (`source_time` и `received_at` не позже decision cutoff), поэтому будущая запись больше не маскирует предыдущую свежую и не создаёт ложный `NO_TRADE`/нулевой капитал.
