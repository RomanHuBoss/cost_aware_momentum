...
> Версия 1.35.1: conditional TIMEOUT estimate сохраняет обученную семантику stop-risk `R` при пересчёте execution plan по текущему ask/bid или depth VWAP. План и acceptance заново проецируют `timeout_return_r` на текущую TP/SL-геометрию; legacy signals без `R` продолжают использовать сохранённую абсолютную ставку.
