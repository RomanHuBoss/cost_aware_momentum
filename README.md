...
> Версия 1.37.0: dynamic training/backtest replay теперь применяет тот же `MAX_SPREAD_BPS`, что и live-публикация. Символы, допущенные широким discovery-порогом `UNIVERSE_MAX_SPREAD_BPS`, но неисполняемые в live, исключаются по immutable snapshot spread evidence; порог связан с promotion-policy contract v3. Защитные пороги не ослаблены.
