...
> Версия 1.7.3: при отсутствии пригодной обученной модели worker продолжает controlled baseline recovery, а trainer после стартовой задержки немедленно запускает bootstrap/recovery training. Несвязанные старые failures больше не блокируют восстановление общим cooldown; повторная техническая ошибка использует отдельный короткий backoff.
