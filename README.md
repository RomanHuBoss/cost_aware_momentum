...
> Версия 1.35.2: ticker snapshots выбираются по latest-prior point-in-time семантике. Future-dated запись больше не маскирует более старую доступную котировку: signal publication, execution planning и API/acceptance требуют одновременно `source_time <= cutoff` и `received_at <= cutoff`, после чего сохраняют строгую stale-age проверку.
