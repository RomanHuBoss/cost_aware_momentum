...
> Версия 1.4.0: добавлен отдельный background trainer. Он периодически переобучает модель на новых подтвержденных свечах, регистрирует immutable candidate, сравнивает его с действующей моделью на одном holdout и автоматически активирует только после прохождения quality gate.
...
## Фоновое переобучение и ручной ML-контур

При обычном запуске `python manage.py run` стартует отдельный trainer-процесс. По умолчанию он:

1. после стартовой задержки проверяет наличие достаточной истории;
2. затем не чаще одного раза в 168 часов переобучает модель на rolling-окне подтвержденных часовых свечей;
...
5. автоматически активирует candidate только если пройдены абсолютные и относительные quality gates;
6. оставляет не прошедшую проверку модель неактивной, не нарушая текущий inference.

Это не `partial_fit` существующего файла. Модель безопасно пересобирается на расширенном/скользящем наборе данных, а предыдущие версии остаются доступными для rollback. CPU-нагрузка обучения вынесена из API и inference worker.

Основные параметры находятся в `.env`: `AUTO_TRAIN_ENABLED`, `AUTO_TRAIN_INTERVAL_HOURS`, `AUTO_TRAIN_MIN_NEW_TIMESTAMPS`, `AUTO_TRAIN_LOOKBACK_DAYS`, `AUTO_TRAIN_MAX_SYMBOLS`, `AUTO_TRAIN_AUTO_ACTIVATE` и пороги quality gate. Для production можно оставить автоматическое обучение, но отключить автоматическую активацию через `AUTO_TRAIN_AUTO_ACTIVATE=false`.
