...
> Версия 1.14.0: execution planning и операторское принятие используют сохраняемые point-in-time Bybit orderbook snapshots, complete-fill VWAP и ограничение ценового воздействия. Частичное или невозможное исполнение блокируется fail-closed; исходный и повторный depth evidence, а также задержка оператора сохраняются для аудита. Исторический backfill depth и causal fill model ещё не реализованы.
...
`MODEL_ENTRY_SPREAD_BPS` задаёт полный bid/ask spread stress для historical labels. Значение делится пополам вокруг следующего hourly open: LONG моделируется по ask-side proxy, SHORT — по bid-side proxy. Изменение этой переменной меняет label geometry; после обновления требуется обучить новый artifact. Research artifact contract 1.13.0 сохраняется без изменения. Release 1.14.0 добавляет live advisory execution evidence через `ORDERBOOK_DEPTH_LEVELS`, `MAX_ORDERBOOK_AGE_SECONDS`, `MAX_VWAP_IMPACT_BPS` и `ORDERBOOK_RETENTION_HOURS`; это не требует retraining, но требует migration `0010_orderbook_exec_evidence` и пересчёта существующих execution plans.
...
Эти ограничения означают, что прохождение unit tests или promotion gate не подтверждает live-edge: historical entry остаётся next-hour open proxy, а live plan использует более поздний executable bid/ask и ручное решение оператора. Release 1.14.0 начинает накапливать point-in-time liquidity snapshots и operator-latency evidence для будущих наблюдений. До появления достаточной forward-истории, historical funding-forecast snapshots и более детальной queue/fill траектории разрыв должен считаться существенным модельным риском.
