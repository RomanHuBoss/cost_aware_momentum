...
> Версия 1.52.11: acceptance validator теперь сам fail-closed отклоняет fresh executable price вне immutable decision-time entry zone, даже если API-слой уже проверил этот контракт и цена выглядит благоприятной по RR/EV. Миграций, новых `.env` variables, order execution capability и API-breaking changes нет.
