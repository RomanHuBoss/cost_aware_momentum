...
> Версия 1.26.3: experiment promotion теперь связывает `READY` trial не только с exact version/SHA-256/horizon, но и с immutable deployment-policy contract: entry spread, leverage/reserve, fees, slippage, stop-gap stress, funding/timeout overrides, EV/RR thresholds и portfolio accounting. Evidence, полученная для другой торговой policy, больше не может активировать artifact.
