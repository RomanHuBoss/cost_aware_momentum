...
> Версия 1.8.25: усилена point-in-time целостность ingestion и inference. Время получения instrument/ticker/funding/OI/account данных фиксируется после ответа Bybit, подтверждённые свечи больше не перезаписываются без явной revision policy, а market cutoff отделён от availability cutoff.
