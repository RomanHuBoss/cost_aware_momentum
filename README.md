...
> Версия 1.26.2: устранён разрыв staged model lifecycle. Background trainer теперь на каждом scheduling cycle повторно проверяет уже зарегистрированный inactive candidate с passed quality gate и `activation_requested=true`; после появления `READY` preregistered family evidence, точно связанной с version/SHA-256/horizon, candidate активируется через общую атомарную fail-closed boundary. Добавлена документированная переменная `AUTO_TRAIN_EXPERIMENT_FAMILY`; migration, risk thresholds и artifact schema не изменены.
