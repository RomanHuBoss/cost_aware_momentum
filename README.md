...
> Версия 1.8.28: допустимая зона входа больше не расширяется наружу при округлении к `tickSize`, приватные read-only GET-запросы Bybit подписываются по точной фактически отправляемой query string, а известные TradFi-типы `stock`, `forex`, `commodity` и `xstocks` исключаются из криптовалютной вселенной по умолчанию.
