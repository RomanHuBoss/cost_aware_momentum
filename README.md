...
> Версия 1.13.0: research pipeline прогрессивно загружает hourly Bybit mark-price candles и применяет консервативный внутригoризонтный isolated-margin proxy. Mark-to-market, MAE/MFE, minimum equity и возможная ликвидация влияют только на realized OOS evidence; будущая mark-траектория не участвует в ex-ante выборе LONG/SHORT, RR, EV или actionability.
...
`MODEL_ENTRY_SPREAD_BPS` задаёт полный bid/ask spread stress для historical labels. Значение делится пополам вокруг следующего hourly open: LONG моделируется по ask-side proxy, SHORT — по bid-side proxy. Изменение этой переменной меняет label geometry; после обновления требуется обучить новый artifact. Версия 1.13.0 дополнительно требует `intrahorizon_margin_schema=bybit-mark-price-hourly-isolated-margin-proxy-v1`, полную hourly mark-price timeline и согласованные `DEFAULT_LEVERAGE`/reserve assumptions. Artifacts 1.12.0 и старше необходимо переобучить после завершения mark-price history backfill.
...
Worker синхронизирует read-only market/account data, instrument specifications, confirmed candles, ticker/funding snapshots и прогрессивно backfill-ит last-price candles, hourly mark-price candles и фактические funding settlement events, после чего строит рекомендации. Неполные или устаревшие данные блокируют публикацию. Для hourly decision последняя confirmed свеча должна закрываться точно в `event_time`; окно с предыдущим часовым close не считается допустимо свежим даже при небольшом возрасте данных.
...
Research backtest моделирует realized-only intrahorizon mark-to-market по точной hourly Bybit mark-price OHLC timeline, учитывает фактически пересечённые funding settlements и консервативно ставит liquidation touch раньше более позднего неупорядоченного last-price TP/SL touch в том же bar. Симулятор остаётся isolated-margin research proxy: он не реконструирует sub-hour mark path, исторические maintenance-margin/risk-tier изменения, liquidation fees, cross/portfolio margin, ADL, страховой фонд или точный fill. Полный historical order book, entry-zone/no-fill, partial fills, point-in-time funding forecast и задержка оператора также отсутствуют.
Эти ограничения означают, что прохождение unit tests или promotion gate не подтверждает live-edge: historical entry остаётся next-hour open proxy, а live plan использует более поздний executable bid/ask и ручное решение оператора. До накопления point-in-time liquidity snapshots, historical funding-forecast snapshots, operator-latency observations и более детальной mark/fill траектории разрыв должен считаться существенным модельным риском.
