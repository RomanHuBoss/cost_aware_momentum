...
> Версия 1.7.0: worker автоматически восстанавливает порядок TP1/SL внутри неоднозначной часовой свечи по точному 1/3/5-минутному public/read-only kline window. Неполный intrabar path оставляет outcome pending; консервативный SL применяется только если TP и SL остаются внутри одного самого мелкого доступного бара.
