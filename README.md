...
> Версия 1.7.6: контрфактическая оценка execution plan fail-closed проверяет qty, stress loss, цены и cost/funding snapshot. Поврежденные `NaN`/`Infinity`/отрицательные значения сохраняются как нулевой `INVALID_INPUT`, а не аварийно прерывают outcome-job и не публикуют фиктивный P&L/R.
