...
> Версия 1.8.1: очередь операторских команд trainer автоматически восстанавливается после аварии процесса. Брошенная `RUNNING`-попытка становится терминальной `FAILED`, создается связанный повтор, audit/outbox сохраняют причину, а позднее завершение старого claim не может перезаписать новый результат.
