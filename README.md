...
> Версия 1.51.0: historical training/backtest geometry теперь использует только доступный на `decision_time` `InstrumentSpecHistory.tick_size`. Directional entry, TP и SL округляются теми же консервативными правилами, что и live-сигнал; future/missing/invalid spec evidence не используется fail-open. Предыдущий 1.50 artifact остаётся запрещённым для runtime, но может один переходный цикл участвовать только в SHA-validated same-holdout incumbent benchmark.
