...
> Версия 1.7.4: risk engine fail-closed проверяет направленную геометрию цен. LONG требует `stop < entry < take-profit`, SHORT — `take-profit < entry < stop`; инвертированный план получает нулевой размер и `BLOCKED_INVALID_INPUT`, а не исполнимую рекомендацию.
