...
> Версия 1.26.5: experiment-selection evidence больше не заполняет разрывы исторических данных вымышленными нулевыми часами. PBO/DSR/dependence используют только часы, покрытые реально наблюдавшимися decision cohorts и их label horizons; legacy return-path evidence блокируется fail-closed.
...
- Research experiment-selection governance prospectively учитывает все backtest-конфигурации одной family, включая failed/open attempts; по выровненным почасовым return paths считает CSCV/PBO, HAC-adjusted Deflated Sharpe и moving-block confidence intervals. Return path содержит genuine no-trade/holding hours внутри наблюдавшихся decision-to-horizon окон, но исключает недоступные календарные разрывы. Неполный/legacy ledger или недостаток независимых временных блоков даёт `BLOCKED`. Normal activation повторно вычисляет report и принимает только `READY` family, чей selected trial относится к exact artifact и exact deployment policy.
