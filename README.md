...
> Версия 1.52.24: приватная operator surface закрыта аутентификацией. Capital profiles, recommendations, manual trades, portfolio risk, detailed readiness/status и SSE outbox больше не доступны анонимно; logout требует authenticated CSRF. Production-конфигурация отклоняет `COOKIE_SECURE=false`. `/health/live`, login, статический UI, glossary и публичный market chart остаются доступными без operator credentials. Alembic schema, model artifacts, risk thresholds и advisory-only Bybit endpoint set не изменены.
