...
> Версия 1.52.6: startup backfill теперь по умолчанию загружает `INITIAL_BACKFILL_BARS=1500` и `sync_candles()` paginates kline-запросы выше одного 1000-row page, чтобы clean install мог сразу достичь текущего 1206-hour default training preflight minimum без снижения ML/econometric gates. Миграций, новых `.env` variables и API changes нет.
...
Release 1.52.6 не добавляет migration, API-breaking changes или новые `.env` variables. Default `INITIAL_BACKFILL_BARS` повышен до `1500`; если в существующем `.env` оставлено `1000`, система продолжит работать, но первый training preflight может ждать progressive history backfill до 1206+ label-eligible часов. `sync_candles()` теперь сам делает несколько kline pages при лимите выше 1000, поэтому startup backfill фактически может загрузить глубину, достаточную для default quality gate. После обновления перезапустите worker и trainer.
