...
> Версия 1.8.2: release tree и `SHA256SUMS` проверяются fail-closed. Отсутствующие, измененные, незарегистрированные или запрещенные артефакты блокируют релиз; восстановлены отсутствовавшие `CHANGELOG.md` и `PATCH_1.8.1.md`.
...
При обновлении с 1.7.11 на 1.7.12 migration и новые `.env` переменные не требуются. Перезапустите API после замены файлов. Ручной close с `fill_time` раньше entry либо последнего сохраненного fill теперь отклоняется с HTTP 422 до изменения сделки; fills с одинаковым timestamp разрешены.

При обновлении с 1.7.12 на 1.8.0 migration и новые `.env` переменные не требуются. Перезапустите API и trainer. В верхней панели откройте «Обучатель»: окно показывает фактическую причину ожидания и позволяет поставить в PostgreSQL очередь `CHECK_NOW` либо, только при отсутствующей/базовой active-модели, `RECOVER_NOW`. Для действий требуется пароль оператора. Recovery пропускает scheduler cooldown, но не минимальную историю, coverage, temporal validation, абсолютные/relative quality gates и guarded activation.

При обновлении с 1.8.0 на 1.8.1 migration и новые `.env` переменные не требуются. Перезапустите API и trainer. Если trainer аварийно завершился после захвата `CHECK_NOW`/`RECOVER_NOW`, новая версия после пяти минут и при stale/missing heartbeat завершает старую попытку как `FAILED`, создает связанный повтор и защищает запись claim-token. Прямое редактирование `ops.job_runs` не требуется.

При обновлении с 1.8.1 на 1.8.2 migration и новые `.env` переменные не требуются. Runtime-процессы можно не перезапускать: изменение относится к release boundary. Перед передачей или публикацией архива выполните `python manage.py release-check`; после осознанных изменений состава пересоздайте manifest командой `python manage.py release-check --write`, затем повторите обычную проверку.
...
python manage.py release-check                               проверить чистоту release tree и SHA256SUMS
python manage.py release-check --write                       атомарно пересоздать manifest для чистого дерева
...
В версии 1.8.0 кнопка «Обучатель» открывает отдельное операторское окно. Оно показывает свежесть heartbeat, текущую фазу, следующую штатную проверку, registry/runtime artifact, progress по timestamps/coverage, последний `model_retraining` и результат quality gate. «Проверить данные сейчас» немедленно повторяет scheduler evaluation. «Запустить восстановительное обучение» доступно только когда active artifact отсутствует, active registry model является baseline либо active-модель не зарегистрирована. Обе команды выполняются фоновым trainer; HTTP request не обучает модель и не блокирует API. Начиная с 1.8.1 зависшая `RUNNING`-команда не блокирует очередь навсегда: при stale/missing heartbeat владельца и возрасте claim не менее пяти минут прежняя попытка фиксируется как `FAILED`, а trainer создает новый связанный `PENDING`-retry.
...
`SHA256SUMS` является каноническим manifest всех файлов release tree, кроме самого manifest. Команда `python manage.py release-check` не требует проектной `.venv` и завершает работу с ошибкой при missing/modified/unlisted file, unsafe manifest path, symlink либо запрещенном артефакте (`.env`, caches, virtual environments, `*.egg-info`, dumps, model files, logs и вложенные archives). `--write` пересоздает manifest атомарно и отказывается работать на загрязненном дереве. Проверка SHA256 не заменяет внешнюю цифровую подпись и доверенный канал доставки.
