...
> Версия 1.8.29: устранено скрытое изменение ATR между обучением и inference, усилен fail-closed контракт модельных артефактов и backtest, запрещено сравнение candidate/incumbent при разной barrier geometry, а quality gate корректно обрабатывает положительный holdout без убыточных периодов.
...
Для горизонта `H` часов капитал делится на `H` равных sleeves. Часовой cohort использует один sleeve и этот капитал не переиспользуется до завершения максимального label horizon. Средние promotion-метрики рассчитываются сначала внутри каждого hourly cohort, затем одинаково по cohort timestamps; количество символов в одном часу не создает дополнительные независимые наблюдения. Auto-activation требует минимум как raw trades, так и независимых `policy_cohorts`. Положительный holdout без отрицательных exit-periods представляет profit factor как математически неограниченный только при явных `gross_gain > 0` и `gross_loss = 0`; отсутствие сделок или неполные метрики остаются fail-closed. Поэтому перекрывающиеся H-часовые returns не компаундятся как последовательные одночасовые сделки и не создают скрытое H-кратное плечо. PnL зачисляется в equity curve в modeled candle exit time. Метрики concurrency считают реально открытые позиции, а не только новые входы в один timestamp.
