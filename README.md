...
> Версия 1.50.0: production feature/probability drift больше не считается только по опубликованным рекомендациям. Каждое успешное вычисление active artifact сохраняется до spread/funding/EV/RR-фильтров в immutable PostgreSQL ledger, поэтому редкая публикация не скрывает сдвиг входных признаков или вероятностей.
