...
> Версия 1.39.0: hourly и universe-catchup inference теперь обновляют read-only account snapshot, order books и ticker batch непосредственно перед публикацией. Долгий startup/backfill больше не создаёт весь universe планов с `BLOCKED_STALE_DATA` из-за заведомо отсутствующего капитала или состарившейся глубины рынка.
