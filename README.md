...
> Версия 1.52.7: progressive history backfill теперь имеет отдельный default для hourly open-interest history: `HISTORY_BACKFILL_OPEN_INTEREST_PAGES_PER_SYMBOL=7`. Это закрывает observed trainer defer `actual_timestamps=326 < required_timestamps=366`, который возникал из-за прежних 2 × 200 OI rows даже при достаточной candle depth. Worker также подавляет повторные stale-hourly логи за один и тот же event hour после terminal `decision_publication_lag_exceeded`. Миграций и API-breaking changes нет.
...
Release 1.52.7 не добавляет migration или API-breaking changes. Добавлена `.env` variable `HISTORY_BACKFILL_OPEN_INTEREST_PAGES_PER_SYMBOL=7`; если в существующем `.env` её нет, встроенный default уже равен 7. Если переменная задана вручную меньше 7, trainer может снова ждать OI history и выдавать `insufficient_walk_forward_history_after_filtering`. После обновления перезапустите worker и trainer.
