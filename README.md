...
> Версия 1.15.0: каждая версия execution plan фиксируется как неизменяемая ex-ante единица operator-selection experiment. Отчёт сопоставляет принятые, отклонённые и оставленные без решения допустимые планы с их counterfactual outcomes и строит временные OOS propensity/ИПВ-диагностики. Это оценка смещения отбора, а не причинный эффект и не доказательство прибыльности.
...
- Каждая plan version в той же транзакции создаёт immutable selection-ledger row с eligibility status, фиксированным набором только ex-ante признаков и SHA-256. Outcomes и решения не записываются в feature snapshot. Отчёты включают `ACCEPT`, `REJECT` и `NO_DECISION`, а не только выбранные сделки.
...
`MODEL_ENTRY_SPREAD_BPS` задаёт полный bid/ask spread stress для historical labels. Значение делится пополам вокруг следующего hourly open: LONG моделируется по ask-side proxy, SHORT — по bid-side proxy. Изменение этой переменной меняет label geometry; после обновления требуется обучить новый artifact. Research artifact contract 1.13.0 сохраняется без изменения. Release 1.14.0 добавил live advisory execution evidence через `ORDERBOOK_DEPTH_LEVELS`, `MAX_ORDERBOOK_AGE_SECONDS`, `MAX_VWAP_IMPACT_BPS` и `ORDERBOOK_RETENTION_HOURS`. Release 1.15.0 не добавляет `.env` параметров и не требует retraining, но требует migration `0011_selection_experiment`; selection evidence накапливается только для новых plan versions после обновления.
...
Эти ограничения означают, что прохождение unit tests или promotion gate не подтверждает live-edge: historical entry остаётся next-hour open proxy, а live plan использует более поздний executable bid/ask и ручное решение оператора. Release 1.14.0 начал накапливать point-in-time liquidity snapshots и operator-latency evidence; release 1.15.0 добавляет prospective selection ledger и диагностику различий между всеми eligible plans и выбранным оператором подмножеством. До появления достаточной forward-истории, historical funding-forecast snapshots и более детальной queue/fill траектории разрыв должен считаться существенным модельным риском.
