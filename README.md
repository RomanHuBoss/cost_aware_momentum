...
> Версия 1.22.0: historical funding replay и `funding_age_fraction` используют funding interval, действовавший в соответствующий момент по `InstrumentSpecHistory`, а не последний известный interval для всей истории. Наблюдаемые смены интервала проходят point-in-time validation; пропущенные settlements по-прежнему блокируют cohort fail-closed. Artifact schemas обновлены, поэтому active/candidate artifacts прежних версий требуется переобучить.
