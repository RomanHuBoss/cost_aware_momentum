...
- Production drift monitoring сравнивает только активную model version с её immutable final-holdout reference: feature/probability PSI, coverage/missingness, maturity-corrected selected-direction calibration и actionability density. Ранние barrier outcomes до полного horizon не входят в calibration; unresolved mature outcomes блокируют evidence. Persisted `CRITICAL` report ставит exact active version на fail-closed quarantine: drift выполняется до inference, новые signals не публикуются, новые/recalculated plans получают `NO_TRADE`, а acceptance старого actionable plan возвращает recalculation conflict. Guard также блокирует stale runtime/signal, если его version не совпадает с current active registry. Карантин переживает worker restart, не очищается отключением новых drift jobs и снимается только другой model version. `BLOCKED` из-за недостатка warm-up evidence не останавливает публикацию, иначе monitor не смог бы накопить собственные prospective snapshots; он продолжает деградировать heartbeat.
