...
> Версия 1.7.5: position sizing fail-closed проверяет все числовые входы. `NaN`, `Infinity`, неположительный капитал/шаг инструмента, отрицательные cost reserves и невалидные margin/cap значения дают нулевой `BLOCKED_INVALID_INPUT`, а не исключение или исполнимый план.
