...
> Версия 1.26.6: experiment-selection evidence использует полный почасовой mark-to-market путь капитала внутри наблюдавшихся decision-to-horizon окон. Внутрисделочная просадка, funding и затраты больше не откладываются целиком до exit; exit-realized v2 evidence блокируется fail-closed.
...
- Research experiment-selection governance prospectively учитывает все backtest-конфигурации одной family, включая failed/open attempts; по выровненным почасовым return paths считает CSCV/PBO, HAC-adjusted Deflated Sharpe и moving-block confidence intervals. Return path содержит genuine no-trade/holding hours внутри наблюдавшихся decision-to-horizon окон, исключает недоступные календарные разрывы и отражает cumulative hourly mark-close MTM: entry fee и conservative slippage признаются в decision time, historical funding — по settlement path, terminal exit fee и barrier/liquidation outcome — в effective exit. Неполный/legacy ledger или недостаток независимых временных блоков даёт `BLOCKED`. Normal activation повторно вычисляет report и принимает только `READY` family, чей selected trial относится к exact artifact и exact deployment policy.
...
`python manage.py train --activate ... --experiment-family <family>` вычисляет оба gate. Fresh artifact обычно ещё не имеет полного preregistered family report, поэтому регистрируется неактивным. Для background trainer после регистрации candidate задайте `AUTO_TRAIN_EXPERIMENT_FAMILY=<family>` и перезапустите trainer: каждая scheduling iteration повторно проверяет exact version/SHA-256/horizon и атомарно активирует candidate только после статуса `READY`. Ручная команда `model-registry activate` остаётся review/rollback-инструментом.
