...
> Версия 1.34.2: universe eligibility snapshot hashes канонизируют `TIMESTAMPTZ` в UTC. PostgreSQL session timezone больше не создаёт ложный `record hash mismatch`, блокирующий trainer preflight/control; настоящая порча immutable evidence по-прежнему блокируется с указанием точного snapshot.
