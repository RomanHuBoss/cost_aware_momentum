...
> Версия 1.6.0: worker автоматически фиксирует контрфактический исход каждого market signal и каждой execution-plan version независимо от решения оператора. TP1/SL/TIMEOUT рассчитываются только по подтвержденным часовым свечам; неполный путь остается нерешенным, а одновременное касание TP/SL трактуется консервативно как SL.
...
Версия 1.6.0 добавляет автоматический post-event журнал, который не зависит от того, принял оператор рекомендацию или нет. После синхронизации очередной закрытой свечи worker:
