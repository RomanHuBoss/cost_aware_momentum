...
> Версия 1.35.0: candidate/live attrition report связывает initial plan blockers с полностью созревшими `SignalOutcome`/`PlanOutcome`. Отчёт показывает TP/SL/TIMEOUT и descriptive counterfactual R по status/stage/reason, исключает ранние outcomes до полного horizon, не видит rows с `resolved_at` после исторического cutoff и блокирует выводы при неполной mature evidence.
