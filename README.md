...
> Версия 1.8.0: в интерфейсе появился отдельный центр «Обучатель моделей» с heartbeat, фазой, причиной ожидания, прогрессом данных, последней попыткой и безопасными командами проверки/восстановления. Команды передаются отдельному trainer через PostgreSQL и не обходят minimum-history, temporal validation или quality gate.
...
В версии 1.8.0 кнопка «Обучатель» открывает отдельное операторское окно. Оно показывает свежесть heartbeat, текущую фазу, следующую штатную проверку, registry/runtime artifact, progress по timestamps/coverage, последний `model_retraining` и результат quality gate. «Проверить данные сейчас» немедленно повторяет scheduler evaluation. «Запустить восстановительное обучение» доступно только когда active artifact отсутствует, active registry model является baseline либо active-модель не зарегистрирована. Обе команды выполняются фоновым trainer; HTTP request не обучает модель и не блокирует API.
