...
С 1.8.11 policy gate имеет отдельный metric contract `exit-time-horizon-sleeves-v2`: realized contribution нормализуется по H перекрывающимся capital sleeves, а candidate/incumbent schema, horizon и sleeve count обязаны совпадать с artifact horizon. Legacy policy metrics не сравниваются молча.
