...
Counterfactual `PlanOutcome` привязан к immutable snapshot конкретной plan version. Если plan был пересчитан из-за цены/профиля, valuation использует сохраненные `entry_price` и `planning_time` этой версии для P&L и funding settlements; signal-level values остаются только legacy fallback. С 1.8.12 open-gap outcome использует цену/время открытия, а stop-gap reserve при известном SL уменьшается на часть gap, уже реализованную в exit price.
Начиная с 1.8.11 новый execution plan не переиспользует cumulative funding из signal: projected settlements пересчитываются от фактического `planning_time` по текущему point-in-time ticker/spec и сохраняются вместе с исходным signal scenario для аудита. При известном ненулевом settlement и неизвестном interval plan блокируется.

С 1.8.12 training labels, counterfactual outcome service, holdout policy и research backtest используют единый open-first barrier contract: валидируется `low <= open/close <= high`, opening adverse stop оценивается по open, favorable TP gap консервативно ограничивается target, а дальнейшая внутрисвечная неоднозначность разрешается lower-timeframe path либо правилом SL.
С 1.8.13 chronological split является частью этого контракта: он требует boolean `exit_at_open`, сохраняет его в final-holdout metadata, а policy/backtest validator запрещает silent fallback к close-time semantics.
...
С 1.8.13 policy gate имел metric contract `exit-time-open-gap-propagated-horizon-sleeves-v4`; с 1.8.14 текущий contract — `exit-time-open-gap-propagated-cohort-weighted-v5`: realized contribution нормализуется по H перекрывающимся capital sleeves, opening exits сохраняют точный open time через dataset split, realized SL использует observed return и residual reserve. Candidate/incumbent schema, horizon и sleeve count обязаны совпадать с artifact horizon; v3 и другие legacy metrics не сравниваются молча.
