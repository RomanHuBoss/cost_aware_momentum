...
- fail-closed при stale/missing market data, migration mismatch и невалидном model artifact; физически отсутствующий active artifact имеет отдельный controlled baseline recovery только в non-production при явном разрешении; counterfactual outcome не создается при разрыве hourly path или неполном обязательном intrabar window;
