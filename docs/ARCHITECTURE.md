...
С 1.8.12 policy gate имеет metric contract `exit-time-realized-gap-horizon-sleeves-v3`: realized contribution нормализуется по H перекрывающимся capital sleeves, opening exits сохраняют точный open time, realized SL использует observed return и residual reserve. Candidate/incumbent schema, horizon и sleeve count обязаны совпадать с artifact horizon; legacy policy metrics не сравниваются молча.
