...
Patch 1.9.3 сохраняет advisory-only/read-only границы, не добавляет order mutations, права API, network exposure, secrets или инфраструктуру. Он закрывает privilege-like escalation внутри risk configuration: профиль капитала больше не может превышать process-wide `MAX_TOTAL_OPEN_RISK_RATE`/`MAX_LEVERAGE`, а legacy-профиль блокируется при planning и acceptance.
