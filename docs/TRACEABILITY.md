...
| Controlled recovery при отсутствующем active artifact | Да с 1.7.2 | baseline только при явном non-production разрешении; DEGRADED diagnostics; trainer bootstrap recovery через absolute gates; invalid/hash mismatch остаются fail-closed |
