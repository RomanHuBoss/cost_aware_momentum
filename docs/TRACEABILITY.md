...
| Bybit malformed/stale list payloads must not masquerade as valid lists, including missing or null `result.list` | `app/bybit/client.py::_require_result_list`, `tests/unit/test_bybit_response_contract_2026_07_09.py` | New targeted regression passed |
...
| Release evidence exists | `CHANGELOG.md`, `PATCH_1.52.16.md`, docs files, `SHA256SUMS` | `scripts/release_integrity.py --write` and verify run after cache cleanup |
