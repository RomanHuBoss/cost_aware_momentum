...
## Исправление `fundingInterval=0` в 1.8.21

После обновления перезапустите worker. В журнале `instrument_sync` больше не должно быть циклической ошибки `Bybit field fundingInterval must be a positive integer`, если нулевое значение относится к `LinearFutures`. Поставочные futures не входят в продуктовый universe и пропускаются до валидации.

Если та же ошибка сохраняется для `contractType=LinearPerpetual`, не подставляйте интервал вручную: сохраните сырой ответ Bybit и остановите использование новых рекомендаций до выяснения причины.

## Повторная проверка внешнего состояния в 1.8.20

Перед `ACCEPTED` система повторно сверяет позиции read-only аккаунта с ручным журналом, требует одновременно `funding_rate` и `next_funding_time`, а также пересчитывает лимит размера как `turnover_24h × 0,0001`. Если биржевая позиция неизвестна журналу, funding/turnover отсутствуют или текущий notional превышает лимит ликвидности, API возвращает HTTP 409. Старый план получает `SUPERSEDED`; рассматривайте только новую версию. При снижении ликвидности она может иметь меньший размер, а при неполных данных останется заблокированной до восстановления синхронизации.

Не подменяйте отсутствующий turnover или funding нулем вручную. Ноль turnover означает отсутствие доказанной исполнимой ликвидности и должен оставлять план заблокированным.
...
Нажатие `Принять` не гарантирует сохранение старого размера. Непосредственно перед фиксацией решения система повторно проверяет текущие капитал и доступную маржу, account reconciliation, индивидуальный и общий risk limits, полную funding timeline, turnover-based liquidity cap, net `R/R`/`EV/R`, а также актуальные `tickSize`, `qtyStep`, min order, max qty и max leverage. При изменении любого критичного входа возвращается HTTP 409 и формируется новая версия плана; используйте только новую версию.
