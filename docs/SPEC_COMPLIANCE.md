...
Состояние на 2026-07-05. Статусы основаны на фактическом коде release 1.26.3, а не на заявлении о полной реализации спецификации.
...
| PBO, Deflated Sharpe, full experiment ledger | Частично реализовано 1.26.3 | Prospective append-only trial ledger, aligned returns, contiguous CSCV/PBO, HAC-adjusted DSR и horizon-floored moving-block intervals сохранены. Новая family до первого `STARTED` требует immutable preregistration. Normal activation требует `READY` selected trial, связанный с exact model version/SHA-256/horizon и immutable deployment-policy contract; central boundary повторно сверяет artifact и fees/slippage/stop-gap/funding-timeout assumptions/EV-RR thresholds/portfolio accounting. Pre-1.18 trials не реконструируются; pre-1.20 families не считаются preregistered; external trusted timestamp, conditional search spaces, automated exclusion coding, experiments outside ledger и автоматический запуск experiment family отсутствуют. |
