...
Состояние на 2026-07-05. Статусы основаны на фактическом коде release 1.26.6, а не на заявлении о полной реализации спецификации.
...
| PBO, Deflated Sharpe, full experiment ledger | Частично реализовано 1.26.6 | Prospective append-only trial ledger, aligned returns, contiguous CSCV/PBO, HAC-adjusted DSR и horizon-floored moving-block intervals сохранены. Experiment return path использует только union реально наблюдавшихся decision-to-horizon окон и теперь отражает cumulative hourly mark-close MTM: entry fee/slippage в decision time, historical funding по settlement path, terminal exit fee и barrier/liquidation outcome в effective exit. Genuine `NO TRADE`/holding hours внутри покрытия остаются нулями, недоступные календарные разрывы исключаются и раскрываются counts. Exit-realized v2 и более старое evidence блокируется. Новая family до первого `STARTED` требует immutable preregistration. Normal activation требует `READY` selected trial, связанный с exact model version/SHA-256/horizon и immutable deployment-policy contract. Pre-1.18 trials не реконструируются; pre-1.20 families не считаются preregistered; external trusted timestamp, conditional search spaces, automated exclusion coding, experiments outside ledger и автоматический запуск experiment family отсутствуют. |
