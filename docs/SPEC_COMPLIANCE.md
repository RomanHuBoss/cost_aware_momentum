...
| Fail-closed operator diagnostics | Implemented, unit tested | structured walk-forward capacity, safe decision-time contract JSON fields, pre-publication stale skip diagnostics, and classified signal-economics skip reasons with per-symbol context |
| Worker stale decision scheduling | Implemented, unit tested | stale hourly/catch-up inference records `decision_publication_lag_exceeded` before publication attempt; repeated hourly and catch-up terminal stale skips are suppressed per event hour |
| Trainer operator diagnostics | Implemented, unit tested | `/api/v1/status` exposes `trainer_control.effective_wait_reason`; UI shows latest persisted training label-building failure when heartbeat wait reason is absent |
| Trainer data-dependent wait diagnostics | Implemented, unit/static checked | rejected bootstrap/recovery candidates report `quality_gate_failed_waiting_for_new_data` or `training_deferred_waiting_for_new_data` with new-labeled-hour progress, remaining threshold and explicit safe-wait copy; previous profile evidence is recovered from trigger or candidate metrics |
