...
Состояние на 2026-07-05. Статусы основаны на фактическом коде release 1.26.5, а не на заявлении о полной реализации спецификации.
...
| PBO, Deflated Sharpe, full experiment ledger | Частично реализовано 1.26.5 | Prospective append-only trial ledger, aligned returns, contiguous CSCV/PBO, HAC-adjusted DSR и horizon-floored moving-block intervals сохранены. Experiment return path использует только union реально наблюдавшихся decision-to-horizon окон: genuine `NO TRADE`/holding hours внутри покрытия остаются нулями, недоступные календарные разрывы исключаются и раскрываются counts. Legacy v1 evidence блокируется. Новая family до первого `STARTED` требует immutable preregistration. Normal activation требует `READY` selected trial, связанный с exact model version/SHA-256/horizon и immutable deployment-policy contract. Pre-1.18 trials не реконструируются; pre-1.20 families не считаются preregistered; external trusted timestamp, conditional search spaces, automated exclusion coding, experiments outside ledger и автоматический запуск experiment family отсутствуют. |
...
| Fail-closed model activation gate | Реализовано 1.26.3 | Normal activation требует passed model quality gate и experiment promotion gate v2. Selected preregistered trial должен совпасть по version/SHA-256/horizon и deployment-policy binding; изменение production fees/slippage/stop-gap/EV-RR thresholds после evidence блокирует activation. Fresh/legacy candidate без binding остаётся inactive. Emergency rollback требует явного flag + reason и сохраняет исходные evidence в audit. |
...
Ограничения: release не добавляет mark-to-market hourly portfolio returns, exact historical orderbook, operator latency или forward profitability evidence. PnL по-прежнему признаётся в modeled exit timestamps, а внутри покрытого holding interval остаётся нулевым до выхода.
...
- promotion gate schema повышена до `model-promotion-experiment-governance-v2`;
